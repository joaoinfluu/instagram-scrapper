...
module.exports = class Insta {
        ...
        return new Promise((resolve, reject) => self.get(username, anonymous ? null : this.sessionId)
            .then(profile => {
                ...
                resolve({
                    ...
                    lastPosts: access ? profile[ 'edge_felix_video_timeline' ][ 'edges' ].map(post => self.partialPost(post)) : null,
    ...
    async _getQueryHashs (){
        if(JSON.stringify(this.queryHashs) !== '{}') {
        ...
        const mainScriptBody = await self.get(Consumer, undefined, false)
        const hashtagScriptBody = await self.get(TagPageContainer, undefined, false)
        const locationScriptBody = await self.get(LocationPageContainer, undefined, false)
